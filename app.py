...
import os
from win32com.client import Dispatch  # pywin32 kutubxonasi (faqat Windows uchun)

# DOC faylni DOCX formatga konvertatsiya qilish funksiyasi
def convert_doc_to_docx(doc_path):
    word = Dispatch("Word.Application")
    doc = word.Documents.Open(doc_path)
    docx_path = f"{os.path.splitext(doc_path)[0]}.docx"
...
def main():
    st.title("📘 Streamlit Test Tizimi")
    st.markdown("<h3 style='text-align: center;'>Test fayllarni yuklang va javob bering:</h3>", unsafe_allow_html=True)

    # Test faylini yuklash
    uploaded_file = st.file_uploader("Shablonni yuklang (DOC yoki DOCX format):", type=["doc", "docx"])
    
    if uploaded_file:
        file_extension = uploaded_file.name.split('.')[-1].lower()
        if file_extension == 'doc':
            with open("temp.doc", "wb") as temp_doc_file:
                temp_doc_file.write(uploaded_file.read())
            docx_path = convert_doc_to_docx("temp.doc")
            questions = load_test_from_docx(docx_path)
            os.remove("temp.doc")  # Ortiqcha faylni o'chirish
        elif file_extension == 'docx':
            questions = load_test_from_docx(uploaded_file)
        else:
            st.error("Faqat DOC va DOCX formatdagi fayllar qabul qilinadi.")
            return
