...
# Variantlar matnini almashtirish funksiyasi
def alter_options_text(options, replacement="Variant -"):
    return [f"{replacement} {i+1}" for i in range(len(options))]
...
def main():
    ...
    if uploaded_file:
        ...
        for i, question in enumerate(questions):
            st.markdown(
                f"<div class='question'><b>{i + 1}. {question['question']}</b></div>", 
                unsafe_allow_html=True
            )
            
            # Asl variantlarni matnini almashtirish
            altered_options = alter_options_text(question['options'])
            ...
            # Foydalanuvchi tanlagan javobni qayta tiklash
            selected_index = altered_options.index(selected_option)
            user_answers[str(i)] = question['options'][selected_index]
        ...
        if st.button("Testni yakunlash"):
            ...
            st.success(f"Test yakunlandi! To'g'ri javoblar soni: {correct_count}/{len(questions)}")
